...
// init first connection
    if(is_null(config::byKey('mqtt_client_id', 'MQTT') ) )  {



      $email = config::byKey('email', 'MQTT', 'nomail');

      log::add('MQTT', 'info', 'Connexion result :'.$email);

      $passwd = config::byKey('passwd', 'MQTT', 'nopasswd');
      $resource_path = realpath(dirname(__FILE__) . '/../../resources/');
      ...
      // get mqtt config
      $url =  "https://api.MQTT.com:443/api/v1/users/auth";
      ...
      curl_setopt($ch, CURLOPT_HTTPHEADER, array(
        ...
        $result = curl_exec($ch);
        log::add('MQTT', 'info', 'Connexion result :'.$result);
...
*/
 ...
 //$client = new Mosquitto\Client('android-uuid/v1');
    ...
    $client = new Mosquitto\Client(config::byKey('mqtt_client_id', 'MQTT', 'noclient'));
    ...
    try {
      ...
      $client->connect(config::byKey('mqtt_endpoint', 'MQTT', '127.0.0.1'), 8883 , 60);
//      $client->connect('a1optpg91s0ydf-2.iot.eu-west-1.amazonaws.com', '8883', 60);

      $topic = 'DB510/'.config::byKey('mac_address','MQTT', '').'/commandOut';
