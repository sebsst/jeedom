...
class worxLandroidS extends eqLogic {
  public static function health() {
    ...
    $server = socket_connect ($socket , config::byKey('worxLandroidSAdress', 'worxLandroidS', '127.0.0.1'), config::byKey('worxLandroidSPort', 'worxLandroidS', '1883'));
    $return[] = array(
...
$client->publish("DB510/".config::byKey('mac_address','worxLandroidS')."/commandIn", "{}", 0, 0);
...
//        log::add('worxLandroidS', 'Debug', ' : ' . $json2_data->cfg->sc->m. ' pour information : ' . $cmdId);
        ...
        self::newInfo($elogic,'errorCode',$json2_data->dat->le,'string');
        self::newInfo($elogic,'errorDescription',self::getErrorDescription($json2_data->dat->le),'string');
        self::newInfo($elogic,'statusCode',$json2_data->dat->ls,'string');
        self::newInfo($elogic,'StatusDescription',self::getStatusDescription($json2_data->dat->ls),'string');
        self::newInfo($elogic,'batteryLevel',$json2_data->dat->bt->p,'numeric');
        self::newInfo($elogic,'Langue',$json2_data->cfg->lg,'string');

        self::newInfo($elogic,'Lastdate',$json2_data->cfg->dt,'string');
        self::newInfo($elogic,'LastTime',$json2_data->cfg->dt,'string');

        self::newInfo($elogic,'Firmware',$json2_data->dat->fw,'string');
        self::newInfo($elogic,'wifiQuality',$json2_data->dat->rsi,'string');
        self::newInfo($elogic,'rainDelay',$json2_data->cfg->rd,'string');

        self::newInfo($elogic,'totalTime',$json2_data->dat->st->wt,'string');
        self::newInfo($elogic,'totalDistance',$json2_data->dat->st->d,'string');
        self::newInfo($elogic,'totalBladeTime',$json2_data->dat->st->b,'string');
        self::newInfo($elogic,'batteryChargeCycle',$json2_data->dat->bt->nr,'string');
        self::newInfo($elogic,'batteryCharging',$json2_data->dat->bt->c,'string');
        self::newInfo($elogic,'batteryVoltage',$json2_data->dat->bt->v,'string');
        self::newInfo($elogic,'batteryTemperature',$json2_data->dat->bt->t,'string');
...
/*
  ...
  */
  ...
  public static function newInfo($elogic,$cmdId,$value,$subtype){
    $cmdlogic = worxLandroidSCmd::byEqLogicIdAndLogicalId($elogic->getId(),$cmdId);
  ...
  public static function publishMosquitto($_id, $_subject, $_message, $_retain) {
    ...
    log::add('worxLandroidS', 'debug', 'Envoi du message ' . $_message . ' vers ' . $_subject. '/'.config::byKey('mqtt_client_id', 'worxLandroidS'));
    ...
    $publish->connect(config::byKey('mqtt_endpoint', 'worxLandroidS', '127.0.0.1'), '8883', 60);
